#CODE CREATED BY GUOYIN
#MODIFIED BY MATIAS TO TAKE INPUTS OF SIZE DIFFERENT THAN BATCH_SIZE
...
def att_emb_ngram_encoder_maxout(x_emb, x_mask, W_class, W_class_tran, opt):
    ...
    print('x_emb_0',x_emb_0.shape)
    x_emb_1 = x_emb_0#tf.multiply(x_emb_0, x_mask) # b * s * e
    print('x_emb_1', x_emb_1.shape)
    x_emb_norm = tf.nn.l2_normalize(x_emb_1, dim=2) # b * s * e
    print('x_emb_norm', x_emb_norm.shape)
    W_class_norm = tf.nn.l2_normalize(W_class_tran, dim = 0) # e * c
    G = tf.contrib.keras.backend.dot(x_emb_norm, W_class_norm) # b * s * c
    x_full_emb = x_emb_0
    print ('G', G.shape)
    Att_v = tf.contrib.layers.conv2d(G, num_outputs=opt.num_class,kernel_size=[opt.ngram], padding='SAME',
...
def partial_softmax(logits, weights, dim, name,):
    print ('partial_softmax logits', logits.shape)
    with tf.name_scope('partial_softmax'):
        ...
        print ('return partial softmax', partial_softmax_score.shape)
